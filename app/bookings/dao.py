...
class BookingDAO(BaseDAO):
    ...
    async def get_external_bookings(cls, **filter_by):
        async with async_session_maker() as session:
            extended_bookings = (
                ...
                .cte("extended_bookings")
            )

            get_extended_bookings_for_user = select(extended_bookings).filter_by(
                **filter_by
            )
    ...
    async def delete(cls, model_id: int, user_id: int):
        async with async_session_maker() as session:
            booking_for_delete = await cls.find_one_or_none(
                id=model_id, user_id=user_id
            )
            ...
            query = delete(Bookings).where(
                and_(Bookings.id == model_id, Bookings.user_id == user_id)
            )
